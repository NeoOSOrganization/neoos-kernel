...
// Armed only by smp_nmi_selftest, and only around the one NMI it sends
// to itself. It is what makes the panic-stop path testable at all: the
// handler's real job is to never return, which a test cannot observe.
static volatile int      nmi_selftest_armed;
static volatile uint64_t nmi_selftest_hits;
...
void nmi_handler(void) {
    if (__atomic_load_n(&nmi_selftest_armed, __ATOMIC_ACQUIRE)) {
        __atomic_fetch_add(&nmi_selftest_hits, 1, __ATOMIC_ACQ_REL);
        return;
    }
    for (;;) { __asm__ volatile ("cli; hlt"); }
}

// Sends this CPU an NMI and checks that nmi_handler actually ran.
//
// This asserts the thing that was previously only assumed: that vector
// 2 REACHES nmi_handler. It did not. The NMI dispatch sat after the
// `vector_number < 32` exception block in isr_handler_inner, so every
// NMI was handled as a CPU exception and nmi_handler was dead code --
// which meant the panic-stop silently did not stop anything.
//
// A regression here fails loudly rather than subtly: with the dispatch
// mis-ordered again, this NMI arrives in ring 0 and the exception path
// halts the boot with a dump naming vector 2.
int smp_nmi_selftest(void) {
    __atomic_store_n(&nmi_selftest_hits, 0, __ATOMIC_RELEASE);
    __atomic_store_n(&nmi_selftest_armed, 1, __ATOMIC_RELEASE);

    lapic_send_nmi(lapic_get_id());

    int ok = 0;
    for (int i = 0; i < 10000000; i++) {
        if (__atomic_load_n(&nmi_selftest_hits, __ATOMIC_ACQUIRE) > 0) { ok = 1; break; }
        __asm__ volatile ("pause");
    }
    __atomic_store_n(&nmi_selftest_armed, 0, __ATOMIC_RELEASE);
    return ok;
}
