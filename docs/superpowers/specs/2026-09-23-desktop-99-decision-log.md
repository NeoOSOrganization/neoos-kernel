...
| D-01-5 | Upstream = SQLite's git mirror as a submodule at `version-3.46.1`; amalgamation generated by SQLite's own build (needs `tclsh`) | vendored amalgamation zip (first draft) | same as every other port (your review); output byte-identical to the published amalgamation |
