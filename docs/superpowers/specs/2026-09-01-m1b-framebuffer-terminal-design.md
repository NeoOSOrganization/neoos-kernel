...
rendering to the linear framebuffer with a NokiaPure bitmap font, so
...
- **Source:** the NokiaPure `.ttf` from
  `github.com/ExertisMicro-P/Bang-Nokia` (`nokia/fonts/NokiaPure`).
  **Proprietary.** Vendored under `third_party/nokiapure/` **with its
  original licence file**. The `.ttf` is a build input only — it is
  never linked into an artefact and never redistributed as a font.
  Only the rasterized 1-bpp bitmap is compiled in. If the licence
  forbids even build-time use, fall back permanently to a
  freely-licensed bitmap font and record the swap here.
- **Cell:** 12×24 px. At 1280×800 → **106 columns × 33 rows** (the
  bottom 8 px row is slack).
- **Host rasterizer:** `tools/rasterize-font.py` (Python +
  `freetype-py`, or `fonttools`): renders U+0020–U+007E plus the VT
  box-drawing set (U+2500-ish — the handful `tput`/`ncurses` use) at
  12×24 into a packed `const uint8_t nokiapure_glyphs[N][12*24/8]`,
  emitted as `userland/term/font_nokiapure.c` + a `font_nokiapure.h`
  with the glyph-index lookup. Mirrors how `kernel/dev/font8x16.c` was
  generated from a PSF1.
- **Fallback:** if `freetype` is absent at build time, the Makefile
  rule regenerates `font_nokiapure.c` from the existing 8×16 PSF
  (scaled/padded to the 12×24 cell) so `make` never breaks on a
  toolchain gap. TERM logs which font it built with.
- The generated `.c` is **checked in** (like `font8x16.c`), so a
  normal build needs no Python.
...
- `userland/term/` : `main.c`, `vt.c` / `vt.h`, `render.c` / `render.h`,
  ...
  `font_nokiapure.c` / `.h` (generated, checked in),
...
- `Makefile`: `TERM.ELF` and `TERMCHILD.ELF` build rules (libneoos +
  ...
  `::BIN/TERMCHILD.ELF`, the font-generation rule with the freetype /
  PSF-fallback branch, `REQUIRED_MARKERS` additions.
...
|---|---|---|
| **M1b-1** | Font pipeline: NokiaPure vendored + licence, `tools/rasterize-font.py`, generated `font_nokiapure.c/.h` checked in, PSF fallback rule. | `make` regenerates the font both with and without freetype; a tiny host harness renders "Ag" and it looks right. |
...
| Font | **NokiaPure** `.ttf` → rasterized to a checked-in bitmap at build time; proprietary, vendored with licence, bitmap-only. |
