...
ifneq ($(filter test uxtest mmapfile dyntest,$(MAKECMDGOALS)),)
...
# mmstress -- concurrent address-space stress test (userland/mmstress.c),
# the .NET-free oracle for the concurrent-request crash investigation.
# Its own boot, nothing else in the INITTAB. Override the CPU count to
...
mmstress: iso disk-image $(USERLAND_BUILD)/MMSTRESS.ELF
	./tools/nexify.sh $(USERLAND_BUILD)/MMSTRESS.ELF $(BUILD_DIR)/mmstress.nex
	@# /usr/tests is an embedfs mount; the FAT root (hd0 -> /) is writable.
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/mmstress.nex ::mmstress.nex
	@printf '%s\n' \
	  '# generated by `make mmstress` -- this test runs alone' \
	  'wait /mmstress.nex' \
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	-@timeout $(BOOT_TIMEOUT) qemu-system-x86_64 $(QEMU_COMMON) \
		-display none -serial file:$(BUILD_DIR)/mmstress.log > /dev/null 2>&1
	@grep -E '^\[mmstress\]|\[fault-audit\]' $(BUILD_DIR)/mmstress.log || true
	@if grep -qE 'PANIC|\[exception\]' $(BUILD_DIR)/mmstress.log; then \
		echo "MMSTRESS: the KERNEL did not survive (smp=$(SMP_CPUS))"; \
		grep -E 'PANIC|\[exception\]|\[fault-audit\]' $(BUILD_DIR)/mmstress.log; exit 1; fi
	@if ! grep -q '\[mmstress\] ALL PASSED' $(BUILD_DIR)/mmstress.log; then \
		echo "MMSTRESS FAILED (smp=$(SMP_CPUS)): no ALL PASSED marker"; tail -8 $(BUILD_DIR)/mmstress.log; exit 1; fi
	@echo "MMSTRESS PASSED (smp=$(SMP_CPUS))"
...
# epolltcp -- epoll-driven TCP accept loop (userland/epolltcp.c), the
# .NET-free oracle for the Kestrel side of the concurrent-request crash.
# Boots alone with a host port-forward, then drives EPOLLTCP_CONC
# concurrent HTTP requests at it from the host and checks every one got
# an answer. `make epolltcp EPOLLTCP_CONC=8`.
EPOLLTCP_PORT ?= 30080
EPOLLTCP_CONC ?= 8
EPOLLTCP_REQS ?= 16

$(USERLAND_BUILD)/EPOLLTCP.ELF: $(USERLAND_DIR)/epolltcp.c $(USERLAND_DIR)/user.ld $(LIBNEOOS_DIR)/lib/crt0.o $(LIBNEOOS_DIR)/lib/libneoos.a
	@mkdir -p $(USERLAND_BUILD)
	$(CC) $(USER_CFLAGS) $(EXTRA_CFLAGS) -T $(USERLAND_DIR)/user.ld -o $@ $(LIBNEOOS_DIR)/lib/crt0.o $(USERLAND_DIR)/epolltcp.c -L$(LIBNEOOS_DIR)/lib -lneoos

.PHONY: epolltcp
epolltcp: iso disk-image $(USERLAND_BUILD)/EPOLLTCP.ELF
	./tools/nexify.sh $(USERLAND_BUILD)/EPOLLTCP.ELF $(BUILD_DIR)/epolltcp.nex
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/epolltcp.nex ::epolltcp.nex
	@printf '%s\n' \
	  '# generated by `make epolltcp` -- this test runs alone' \
	  'wait /epolltcp.nex' \
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@rm -f $(BUILD_DIR)/epolltcp.log
	@# The guest must be running while the host drives it, so qemu goes
	@# to the background and the driver waits for the "listening" line
	...
	@tools/epolltcp-drive.sh $(EPOLLTCP_PORT) $(EPOLLTCP_CONC) $(EPOLLTCP_REQS) \
		$(BUILD_DIR)/epolltcp.log; rc=$$?; \
	 kill `cat $(BUILD_DIR)/epolltcp.qemu.pid` 2>/dev/null || true; \
	 wait 2>/dev/null || true; \
	 grep -E '^\[epolltcp\]|\[fault-audit\]|\[usrflt\]' $(BUILD_DIR)/epolltcp.log || true; \
	 if grep -qE 'PANIC|\[exception\]' $(BUILD_DIR)/epolltcp.log; then \
		echo "EPOLLTCP: the KERNEL did not survive"; \
		grep -E 'PANIC|\[exception\]' $(BUILD_DIR)/epolltcp.log; exit 1; fi; \
	 exit $$rc

# epollet -- EPOLLET conformance (userland/epollet.c). Unlike epolltcp
# this needs no host driver and no network: a pipe and a socketpair are
# enough to pin edge-triggered semantics down, which is what .NET's
# SocketAsyncEngine asks for on every socket it opens. Its own boot,
# nothing else in the INITTAB.
$(USERLAND_BUILD)/EPOLLET.ELF: $(USERLAND_DIR)/epollet.c $(USERLAND_DIR)/user.ld $(LIBNEOOS_DIR)/lib/crt0.o $(LIBNEOOS_DIR)/lib/libneoos.a
	@mkdir -p $(USERLAND_BUILD)
	$(CC) $(USER_CFLAGS) $(EXTRA_CFLAGS) -T $(USERLAND_DIR)/user.ld -o $@ $(LIBNEOOS_DIR)/lib/crt0.o $(USERLAND_DIR)/epollet.c -L$(LIBNEOOS_DIR)/lib -lneoos

.PHONY: epollet
epollet: iso disk-image $(USERLAND_BUILD)/EPOLLET.ELF
	./tools/nexify.sh $(USERLAND_BUILD)/EPOLLET.ELF $(BUILD_DIR)/epollet.nex
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/epollet.nex ::epollet.nex
	@printf '%s\n' \
	  '# generated by `make epollet` -- this test runs alone' \
	  'wait /epollet.nex' \
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	-@timeout $(BOOT_TIMEOUT) qemu-system-x86_64 $(QEMU_COMMON) \
		-display none -serial file:$(BUILD_DIR)/epollet.log > /dev/null 2>&1
	@grep -E '^\[epollet\]|\[fault-audit\]' $(BUILD_DIR)/epollet.log || true
	@if grep -qE 'PANIC|\[exception\]' $(BUILD_DIR)/epollet.log; then \
		echo "EPOLLET: the KERNEL did not survive"; \
		grep -E 'PANIC|\[exception\]|\[fault-audit\]' $(BUILD_DIR)/epollet.log; exit 1; fi
	@if ! grep -q '\[epollet\] ALL PASSED' $(BUILD_DIR)/epollet.log; then \
		echo "EPOLLET FAILED: no ALL PASSED marker"; tail -8 $(BUILD_DIR)/epollet.log; exit 1; fi
	@echo "EPOLLET PASSED: edge-triggered epoll behaves like Linux's"

# uxtest -- the GUI stack's kernel IPC conformance test
# (userland/uxtest.c): ftruncate, memfd_create, AF_UNIX and SCM_RIGHTS,
# none of which existed before G3. No host driver and no network; its
# own boot, nothing else in the INITTAB.
$(USERLAND_BUILD)/UXTEST.ELF: $(USERLAND_DIR)/uxtest.c $(USERLAND_DIR)/user.ld $(LIBNEOOS_DIR)/lib/crt0.o $(LIBNEOOS_DIR)/lib/libneoos.a
	@mkdir -p $(USERLAND_BUILD)
	$(CC) $(USER_CFLAGS) $(EXTRA_CFLAGS) -T $(USERLAND_DIR)/user.ld -o $@ $(LIBNEOOS_DIR)/lib/crt0.o $(USERLAND_DIR)/uxtest.c -L$(LIBNEOOS_DIR)/lib -lneoos

.PHONY: uxtest
uxtest: iso disk-image $(USERLAND_BUILD)/UXTEST.ELF
	./tools/nexify.sh $(USERLAND_BUILD)/UXTEST.ELF $(BUILD_DIR)/uxtest.nex
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/uxtest.nex ::uxtest.nex
	@printf '%s\n' \
	  '# generated by `make uxtest` -- this test runs alone' \
	  'wait /uxtest.nex' \
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	-@timeout $(BOOT_TIMEOUT) qemu-system-x86_64 $(QEMU_COMMON) \
		-display none -serial file:$(BUILD_DIR)/uxtest.log > /dev/null 2>&1
	@grep -E '^\[uxtest\]|\[fault-audit\]' $(BUILD_DIR)/uxtest.log || true
	@if grep -qE 'PANIC|\[exception\]' $(BUILD_DIR)/uxtest.log; then \
		echo "UXTEST: the KERNEL did not survive"; \
		grep -E 'PANIC|\[exception\]|\[fault-audit\]' $(BUILD_DIR)/uxtest.log; exit 1; fi
	@if ! grep -q '\[uxtest\] ALL PASSED' $(BUILD_DIR)/uxtest.log; then \
		echo "UXTEST FAILED: no ALL PASSED marker"; tail -12 $(BUILD_DIR)/uxtest.log; exit 1; fi

# ---- spvtest: boots neoos-tinygl's SPIR-V shader-stage self-test ----
# neoos-tinygl is a sibling repo, built separately (see
# docs/superpowers/specs/2026-09-20-tinygl-spirv-shader-stage-design.md);
# this target just mcopies its already-built spvtest.nex onto the disk
# and boots it alone, the same way uxtest boots its own UXTEST.ELF.
...
# build.
WM_DIR ?= ../neoos-wm
# The desktop targets get more guest RAM than QEMU's 128MB default: the
# compositor alone maps Mesa's softpipe (libOSMesa, ~12MB) plus its
# screen-sized buffers, and one GL client beside it (EGL + swrast_dri,
# another ~14MB of .so's plus its own buffers) exhausts 128MB --
# measured: the EGL triangle client dies of OOM next to wm at the
# default, and passes at 512MB.
DESKTOP_MEM ?= 512
QEMU_DESKTOP := $(QEMU_COMMON) -m $(DESKTOP_MEM)
MESA_DIR ?= ../neoos-mesa
...
wm: iso disk-image
	...
	@printf '%s\n' \
	  '# generated by `make wm`' \
	  'spawn /wm.nex 90' \
	  'wait /wmdemo.nex' \
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@tools/boot_until.sh $(BUILD_DIR)/wm.log '\[wm\] surface mapped' $(BOOT_TIMEOUT) -- $(QEMU_DESKTOP) -display none
	@grep -E '^\[wm\]|^\[wmdemo\]|\[fault-audit\]' $(BUILD_DIR)/wm.log || true
	@if grep -qE 'PANIC|\[exception\]' $(BUILD_DIR)/wm.log; then \
		echo "WM: the KERNEL did not survive"; \
		grep -E 'PANIC|\[exception\]' $(BUILD_DIR)/wm.log; exit 1; fi
	@if ! grep -q '\[wm\] surface mapped' $(BUILD_DIR)/wm.log; then \
		echo "WM FAILED: no surface was ever mapped"; tail -20 $(BUILD_DIR)/wm.log; exit 1; fi

# `make wm-glass` boots the compositor with TWO wmdemo processes: one
# plain (the gradient window), one in glass mode (wmdemo.nex's argv[2]
# == "glass", see neoos-wm/wmdemo.c) -- wm_conn holds exactly one
# surface per connection, so the glass demo window has to be a second
# process, not a second surface on the first. Smoke-tests
# docs/superpowers/specs/2026-09-20-wm-glass-lensing-design.md end to end.
.PHONY: wm-glass
wm-glass: iso disk-image
	...
	@printf '%s\n' \
	  ...
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@COUNT=2 tools/boot_until.sh $(BUILD_DIR)/wm-glass.log '\[wm\] surface mapped' $(BOOT_TIMEOUT) -- $(QEMU_DESKTOP) -display none
...
wm-cursor: iso disk-image
	...
	@printf '%s\n' \
	  ...
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@# wm.nex never exits on its own once idle (its frame budget only
	@# counts real repaints), so the run is ended by boot_until.sh the
	@# moment the last line these checks need is in the log --
	@# $(BOOT_TIMEOUT) is only the hang bound, never a wait.
	@tools/boot_until.sh $(BUILD_DIR)/wm-cursor.log '\[wm\] surface mapped' $(BOOT_TIMEOUT) -- $(QEMU_DESKTOP) -display none
	...
	@DESKTOP_MEM=$(DESKTOP_MEM) tools/screenshot.sh $(BUILD_DIR)/wm-cursor-screen.ppm 90 '\[wm\] surface mapped'
...
wm-cursor-fallback: iso disk-image
	...
	@printf '%s\n' \
	  ...
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@# Ended the same way as wm-cursor: on the log line, not a timer.
	@tools/boot_until.sh $(BUILD_DIR)/wm-cursor-fallback.log '\[wm\] surface mapped' $(BOOT_TIMEOUT) -- $(QEMU_DESKTOP) -display none
	@# Put the cursor back: disk.img is shared by every other target
	@# (and by `make run`), and all of them expect the themed pointer.
	mcopy -o -i $(DISK_IMG) $(WM_DIR)/assets/icons/gm_cursors/cursors/default ::usr/share/icons/gm_cursors/cursors/default
...
# `make wm-shot` proves the compositor PAINTS, which the serial log
...
wm-shot: iso disk-image
	...
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/wmdemo.nex ::wmdemo.nex
	@printf '%s\n' \
	  '# generated by `make wm-shot`' \
	  'spawn /wm.nex 1000000' \
	  'spawn /wmdemo.nex 1000000' \
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@DESKTOP_MEM=$(DESKTOP_MEM) tools/screenshot.sh $(BUILD_DIR)/wm-screen.ppm 90 '\[wm\] surface mapped'
...
wm-taskbar: iso disk-image
	...
	@printf '%s\n' \
	  ...
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@# Every wait below is on a log line, never a fixed sleep. The log
	@# is deleted first: QEMU truncates it only once it opens it, so a
	@# stale one from the last run would satisfy the first grep
	@# instantly and fire the clicks before the compositor is even
	@# reading the mouse (the old "Start click missed" failure).
	@rm -f $(BUILD_DIR)/wm-taskbar.log
	...
	for i in $$(seq 1 240); do \
		grep -q '\[wm\] surface mapped' $$LOG 2>/dev/null && break; \
		sleep 0.5; \
	done; \
	tools/inject_click.sh "$$SOCK" -2000 2000 '\[startmenu\] mapped' $$LOG; \
	tools/screenshot_settled.sh "$$SOCK" $(BUILD_DIR)/wm-taskbar-menu-open.ppm; \
	tools/inject_click.sh "$$SOCK" 2000 -2000 '\[startmenu\] closing on focus loss' $$LOG; \
	tools/screenshot_settled.sh "$$SOCK" $(BUILD_DIR)/wm-taskbar-menu-closed.ppm; \
...
# ---- the C# + LVGL hello-world window --------------------------------
#
# The application lives in the neoos-lvgl repo (it needs the musl
# toolchain and dotnet, neither of which this Makefile owns), so this
# target consumes the already-published binary rather than building it.
# Publish it with neoos-lvgl/csharp/Hello/publish.sh first.
NEOOS_LVGL_DIR ?= $(HOME)/projects/personal/neoos-lvgl
...
hello: iso disk-image
	...
	@printf '%s\n' \
	  '# generated by `make hello`' \
	  'spawn /wm.nex 1000000' \
	  'spawn /hello.nex 1000000' \
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@DESKTOP_MEM=$(DESKTOP_MEM) tools/screenshot.sh $(BUILD_DIR)/hello-screen.ppm 120 '\[wm\] surface mapped'
	@grep -E '^\[wm\]|^\[hello\]|\[shim\] ENOSYS' $(BUILD_DIR)/screenshot.log | head -30 || true

# `make hello-run` -- the same image as `make hello`, but in a REAL
# QEMU window you can click in, instead of a headless screenshot.
#
...
hello-run: iso disk-image
	...
	qemu-system-x86_64 $(QEMU_DESKTOP) -serial stdio

# `make wm-run` -- the compositor and the C demo client, no dotnet
# needed. Useful when you want to see the window system without

# `make desktop` -- boot straight to the desktop: the compositor and the
# taskbar (whose Start button spawns the start menu), in a real QEMU
# window you can click in. Everything is staged fresh from WM_DIR and
# MESA_DIR each time, so it never boots a stale compositor. KVM=1 for
# hardware virtualisation (much snappier); the default is the TCG
# reference machine. Click into the window to grab the mouse;
# ctrl+alt+g releases it.
.PHONY: desktop
desktop: iso disk-image
	@test -f $(WM_ELF) || { echo "desktop: $(WM_ELF) missing -- build neoos-wm first (WM_DIR=$(WM_DIR))"; exit 1; }
	@test -f $(TASKBAR_ELF) || { echo "desktop: $(TASKBAR_ELF) missing -- build neoos-wm first (WM_DIR=$(WM_DIR))"; exit 1; }
	@test -f $(STARTMENU_ELF) || { echo "desktop: $(STARTMENU_ELF) missing -- build neoos-wm first (WM_DIR=$(WM_DIR))"; exit 1; }
	@test -d $(MESA_DIR)/build-output-runtime-libs || { echo "desktop: no Mesa runtime libs -- run $(MESA_DIR)/build.sh first"; exit 1; }
	./tools/nexify.sh $(WM_ELF) $(BUILD_DIR)/wm.nex
	./tools/nexify.sh $(TASKBAR_ELF) $(BUILD_DIR)/taskbar.nex
	./tools/nexify.sh $(STARTMENU_ELF) $(BUILD_DIR)/startmenu.nex
	mmd -D s -i $(DISK_IMG) ::usr/local/bin 2>/dev/null || true
	...
	@printf '%s\n' \
	  ...
	  'spawn /usr/local/bin/wm.nex' \
	  'spawn /usr/local/bin/taskbar.nex' \
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@# NO -device usb-mouse: the compositor reads the PS/2 mouse (see wm-run).
	qemu-system-x86_64 $(QEMU_DESKTOP) -serial stdio
...
# mmapfile -- file mapping and pread conformance for the dynamic
# linking milestone (userland/mmapfile.c). Its own boot, nothing else
# in the INITTAB. Built with -DNEOOS_TEST_HOOKS like uxtest: the
# concurrent-fault leak check reads the free-frame count through the
# test hook, and without it that check compares -ENOSYS to itself.
$(USERLAND_BUILD)/MMAPFILE.ELF: $(USERLAND_DIR)/mmapfile.c $(USERLAND_DIR)/user.ld $(LIBNEOOS_DIR)/lib/crt0.o $(LIBNEOOS_DIR)/lib/libneoos.a
	@mkdir -p $(USERLAND_BUILD)
	$(CC) $(USER_CFLAGS) $(EXTRA_CFLAGS) -T $(USERLAND_DIR)/user.ld -o $@ $(LIBNEOOS_DIR)/lib/crt0.o $(USERLAND_DIR)/mmapfile.c -L$(LIBNEOOS_DIR)/lib -lneoos

.PHONY: mmapfile
mmapfile: iso disk-image $(USERLAND_BUILD)/MMAPFILE.ELF
	./tools/nexify.sh $(USERLAND_BUILD)/MMAPFILE.ELF $(BUILD_DIR)/mmapfile.nex
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/mmapfile.nex ::mmapfile.nex
	@printf '%s\n' \
	  '# generated by `make mmapfile` -- this test runs alone' \
	  'wait /mmapfile.nex' \
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	-@timeout $(BOOT_TIMEOUT) qemu-system-x86_64 $(QEMU_COMMON) \
		-display none -serial file:$(BUILD_DIR)/mmapfile.log > /dev/null 2>&1
	@grep -E '^\[dyn\]|\[fault-audit\]' $(BUILD_DIR)/mmapfile.log || true
	@if grep -qE 'PANIC|\[exception\]' $(BUILD_DIR)/mmapfile.log; then \
		echo "MMAPFILE: the KERNEL did not survive"; \
		grep -E 'PANIC|\[exception\]|\[fault-audit\]' $(BUILD_DIR)/mmapfile.log; exit 1; fi
	@if ! grep -q '\[dyn\] ALL PASSED' $(BUILD_DIR)/mmapfile.log; then \
		echo "MMAPFILE FAILED: no ALL PASSED marker"; tail -12 $(BUILD_DIR)/mmapfile.log; exit 1; fi

# ---- dyntest: the dynamic-linking milestone --------------------------
#
# Built with the HOSTED toolchain, not $(CC): x86_64-elf-gcc is
...
$(BUILD_DIR)/dynlib.so: $(USERLAND_DIR)/dynlib.c
	@mkdir -p $(BUILD_DIR)
	$(NEOOS_HOSTED)gcc -O2 -fPIC -shared -o $@ $(USERLAND_DIR)/dynlib.c
...
$(BUILD_DIR)/dyntest.elf: $(USERLAND_DIR)/dyntest.c
	...
	$(NEOOS_HOSTED)gcc -O2 -o $@ $(USERLAND_DIR)/dyntest.c -ldl

.PHONY: dyntest
dyntest: iso disk-image $(BUILD_DIR)/dyntest.elf $(BUILD_DIR)/dynlib.so
	./tools/nexify.sh $(BUILD_DIR)/dyntest.elf $(BUILD_DIR)/dyntest.nex
	...
	@printf '%s\n' \
	  '# generated by `make dyntest` -- this test runs alone' \
	  'wait /dyntest.nex' \
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	-@timeout $(BOOT_TIMEOUT) qemu-system-x86_64 $(QEMU_COMMON) \
		-display none -serial file:$(BUILD_DIR)/dyntest.log > /dev/null 2>&1
	@grep -E '^\[dyn\]|^\[elf\]|\[fault-audit\]|\[shim\] ENOSYS' $(BUILD_DIR)/dyntest.log || true
	@if grep -qE 'PANIC|\[exception\]' $(BUILD_DIR)/dyntest.log; then \
		echo "DYNTEST: the KERNEL did not survive"; \
		grep -E 'PANIC|\[exception\]' $(BUILD_DIR)/dyntest.log; exit 1; fi
	@if ! grep -q '\[dyn\] ALL PASSED' $(BUILD_DIR)/dyntest.log; then \
		echo "DYNTEST FAILED: no ALL PASSED marker"; tail -20 $(BUILD_DIR)/dyntest.log; exit 1; fi

# m0test -- userland checks for the desktop M0 kernel prerequisites
# (docs/superpowers/plans/2026-09-23-desktop-m0-kernel-prereqs.md): record
...
m0test: iso disk-image $(BUILD_DIR)/m0test.elf
	./tools/nexify.sh $(BUILD_DIR)/m0test.elf $(BUILD_DIR)/m0test.nex
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/m0test.nex ::m0test.nex
	@printf '%s\n' '# generated by `make m0test`' 'wait /m0test.nex' > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@tools/boot_until.sh $(BUILD_DIR)/m0test.log '^PASS m0test|FAILED[[:space:]]*$$' $(BOOT_TIMEOUT) -- $(QEMU_COMMON) -display none
	@grep -aE '^m0test:|^PASS m0test|PANIC|\[exception\]' $(BUILD_DIR)/m0test.log || true
	@grep -aq '^PASS m0test' $(BUILD_DIR)/m0test.log || { echo "M0TEST FAILED"; exit 1; }
...
blkdevtest: iso disk-image $(BUILD_DIR)/blkdevtest.elf
	...
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@tools/boot_until.sh $(BUILD_DIR)/blkdevtest.log '^PASS blkdevtest|^blkdevtest: FAILED' $(BOOT_TIMEOUT) -- $(QEMU_COMMON) -display none
...
# lines plus QEMU EXITING (ACPI power-off, or -no-reboot turning the
# reset into an exit) -- boot_until returns when QEMU does.
$(BUILD_DIR)/powertest.elf: $(USERLAND_DIR)/powertest.c
	@mkdir -p $(BUILD_DIR)
	$(NEOOS_HOSTED)gcc -O2 -static -o $@ $(USERLAND_DIR)/powertest.c

.PHONY: powertest powertest-reboot
powertest powertest-reboot: iso disk-image $(BUILD_DIR)/powertest.elf
	./tools/nexify.sh $(BUILD_DIR)/powertest.elf $(BUILD_DIR)/powertest.nex
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/powertest.nex ::powertest.nex
	@printf '%s\n' '# generated by `make $@`' \
	  'spawn /powertest.nex $(if $(filter powertest-reboot,$@),--reboot,)' \
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@tools/boot_until.sh $(BUILD_DIR)/$@.log 'PT_NEVER_MATCHES' $(BOOT_TIMEOUT) -- $(QEMU_COMMON) -display none
	@grep -aE '^powertest:|^\[init\]|\[kernel\] shutdown|PANIC|\[exception\]' $(BUILD_DIR)/$@.log || true
	@if [ "$@" = powertest ]; then \
	  grep -aq '\[init\] power off requested' $(BUILD_DIR)/$@.log && \
	  grep -aq '\[init\] powering off' $(BUILD_DIR)/$@.log && \
	  grep -aq '\[kernel\] shutdown requested' $(BUILD_DIR)/$@.log || { echo "POWERTEST FAILED"; exit 1; }; \
	else \
	  grep -aq '\[init\] reboot requested' $(BUILD_DIR)/$@.log && \
	  grep -aq '\[init\] restarting' $(BUILD_DIR)/$@.log && \
	  ! grep -aq '\[kernel\] shutdown requested' $(BUILD_DIR)/$@.log || { echo "POWERTEST FAILED"; exit 1; }; \
	fi
	@echo "$@: PASSED"

# sqlitetest -- SQLite on NeoOS (desktop M1): the neoos-sqlite port
# (SQLITE_DIR, built first: `make -C ../neoos-sqlite`), nsql, and the
# kernel's record locks under a real database. Two boots on the same
# disk image: phase 1 (the sqlite3 CLI, migrations, locking, concurrent
# writers, crash rollback), then phase 2 after a reboot (persistence).
SQLITE_DIR ?= ../neoos-sqlite
$(BUILD_DIR)/sqlitetest.elf: $(USERLAND_DIR)/sqlitetest.c $(SQLITE_DIR)/build-output/lib/libsqlite3.a
	@mkdir -p $(BUILD_DIR)
	$(NEOOS_HOSTED)gcc -O2 -static -I$(SQLITE_DIR)/build-output/include -o $@ $(USERLAND_DIR)/sqlitetest.c \
		-L$(SQLITE_DIR)/build-output/lib -lnsql -lsqlite3 -lpthread -lm

$(SQLITE_DIR)/build-output/lib/libsqlite3.a:
	@echo "error: build neoos-sqlite first: make -C $(SQLITE_DIR)" >&2; exit 1

.PHONY: sqlitetest
sqlitetest: iso disk-image $(BUILD_DIR)/sqlitetest.elf
	./tools/nexify.sh $(BUILD_DIR)/sqlitetest.elf $(BUILD_DIR)/sqlitetest.nex
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/sqlitetest.nex ::sqlitetest.nex
	mmd -D s -i $(DISK_IMG) ::usr/local 2>/dev/null || true
	mmd -D s -i $(DISK_IMG) ::usr/local/bin 2>/dev/null || true
	mcopy -o -i $(DISK_IMG) $(SQLITE_DIR)/build/sqlite3.nex ::usr/local/bin/sqlite3.nex
	@printf '%s\n' 'CREATE TABLE x(a);' "INSERT INTO x VALUES('cli-ok');" 'SELECT a FROM x;' > $(BUILD_DIR)/disk-src/cli.sql
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/cli.sql ::cli.sql
	@printf '%s\n' '# generated by `make sqlitetest` (phase 1)' \
	  'wait /usr/local/bin/sqlite3.nex -init /cli.sql /root/cli.db .quit' \
	  'wait /sqlitetest.nex' > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@tools/boot_until.sh $(BUILD_DIR)/sqlitetest.log '^PASS sqlitetest phase1|FAILED[[:space:]]*$$' $(BOOT_TIMEOUT) -- $(QEMU_COMMON) -display none
	@printf '%s\n' '# generated by `make sqlitetest` (phase 2)' 'wait /sqlitetest.nex phase2' > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@tools/boot_until.sh $(BUILD_DIR)/sqlitetest2.log '^PASS sqlitetest[[:space:]]*$$|FAILED[[:space:]]*$$' $(BOOT_TIMEOUT) -- $(QEMU_COMMON) -display none
	@grep -aE '^sqlitetest:|^PASS sqlitetest|^cli-ok|\[nsql\]|PANIC|\[exception\]' $(BUILD_DIR)/sqlitetest.log $(BUILD_DIR)/sqlitetest2.log || true
	@grep -aq '^cli-ok' $(BUILD_DIR)/sqlitetest.log || { echo "SQLITETEST FAILED: the sqlite3 CLI did not answer"; exit 1; }
	@grep -aq '^PASS sqlitetest phase1' $(BUILD_DIR)/sqlitetest.log || { echo "SQLITETEST FAILED (phase 1)"; exit 1; }
	@grep -aqE '^PASS sqlitetest[[:space:]]*$$' $(BUILD_DIR)/sqlitetest2.log || { echo "SQLITETEST FAILED (phase 2)"; exit 1; }
...
hrtest: iso disk-image $(BUILD_DIR)/hrtest.elf
	...
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@tools/boot_until.sh $(BUILD_DIR)/hrtest.log '^PASS hrtest|FAILED[[:space:]]*$$' $(BOOT_TIMEOUT) -- $(QEMU_COMMON) -display none
