...
# wmwait -- a fixed head-start delay for the wm-* dev-convenience
# targets below, so wmdemo.nex doesn't race WM.ELF's now-dynamic-linked
# startup. See wmwait.c's own top-of-file comment.
$(USERLAND_BUILD)/WMWAIT.ELF: $(USERLAND_DIR)/wmwait.c $(USERLAND_DIR)/user.ld $(LIBNEOOS_DIR)/lib/crt0.o $(LIBNEOOS_DIR)/lib/libneoos.a
	@mkdir -p $(USERLAND_BUILD)
	$(CC) $(USER_CFLAGS) -T $(USERLAND_DIR)/user.ld -o $@ $(LIBNEOOS_DIR)/lib/crt0.o $(USERLAND_DIR)/wmwait.c -L$(LIBNEOOS_DIR)/lib -lneoos
...
.PHONY: wm
wm: iso disk-image $(USERLAND_BUILD)/WMWAIT.ELF
	...
	./tools/nexify.sh $(USERLAND_BUILD)/WMWAIT.ELF $(BUILD_DIR)/wmwait.nex
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/wm.nex ::wm.nex
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/wmdemo.nex ::wmdemo.nex
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/wmwait.nex ::wmwait.nex
	@printf '%s\n' \
	...
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	-@timeout $(BOOT_TIMEOUT) qemu-system-x86_64 $(QEMU_COMMON) \
		-display none -serial file:$(BUILD_DIR)/wm.log > /dev/null 2>&1
...
.PHONY: wm-glass
wm-glass: iso disk-image $(USERLAND_BUILD)/WMWAIT.ELF
...
.PHONY: wm-cursor
wm-cursor: iso disk-image $(USERLAND_BUILD)/WMWAIT.ELF
	...
	@printf '%s\n' \
	  ...
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@# wm.nex's own frame budget ("90" above) only counts a frame on an
	@# actual repaint, which only happens on new screen damage -- once
	@# wmdemo disconnects, nothing generates further damage in this
	@# headless run, so wm.nex never reaches its own exit condition and
	@# sits blocked in poll() forever. The full test output (cursor
	@# load, surface mapped, wmdemo done) is already in the log within
	@# the first few seconds of boot, so a short local timeout -- not
	@# $(BOOT_TIMEOUT)'s 150s hang-detector ceiling -- is what actually
	@# ends this run; 30s leaves comfortable margin over that.
	-@timeout 30 qemu-system-x86_64 $(QEMU_COMMON) \
...
wm-cursor-fallback: iso disk-image $(USERLAND_BUILD)/WMWAIT.ELF
	...
	@printf '%s\n' \
	  ...
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@# Same short local timeout as wm-cursor, same reason: wm.nex's
	@# frame budget never fires once idle, so a short bound -- not
	@# $(BOOT_TIMEOUT)'s 150s hang-detector ceiling -- is what ends this
	@# run once the fallback path has already been proven in the log.
	-@timeout 30 qemu-system-x86_64 $(QEMU_COMMON) \
...
.PHONY: wm-shot
wm-shot: iso disk-image $(USERLAND_BUILD)/WMWAIT.ELF
	...
	@printf '%s\n' \
	  ...
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@tools/screenshot.sh $(BUILD_DIR)/wm-screen.ppm 24
...
.PHONY: wm-taskbar
wm-taskbar: iso disk-image $(USERLAND_BUILD)/WMWAIT.ELF
	...
	for i in $$(seq 1 60); do \
		grep -q '\[taskbar\] ready' $(BUILD_DIR)/wm-taskbar.log 2>/dev/null && break; \
		sleep 0.5; \
	done; \
	tools/inject_click.sh "$$SOCK" -2000 2000; \
...
hello: iso disk-image
	...
	@printf '%s\n' \
	  ...
	  > $(BUILD_DIR)/disk-src/INITTAB.solo
	mcopy -o -i $(DISK_IMG) $(BUILD_DIR)/disk-src/INITTAB.solo ::etc/inittab
	@tools/screenshot.sh $(BUILD_DIR)/hello-screen.ppm 30
